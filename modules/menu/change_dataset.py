...
def change_dataset(root=None, selected_file=None):
    ...
    def delete_metadata():
        ...
        if os.path.exists(log_file_path):
            df_log = pd.read_excel(log_file_path)
            if {'identifier', 'metadata_status'}.issubset(df_log.columns):
                identifier = normalize_log_identifier(df_current_data.loc[df_current_data['Metadata Property'] == 'has_identifier', 'Metadata Value'].values[0])
                df_log['identifier'] = df_log['identifier'].fillna('').astype(str).str.strip()
                df_log['identifier'] = df_log['identifier'].str.replace(r'\.xlsx$', '', regex=True)
    ...
    #Titel 1. Zeile
    label_title = tk.Label(newdatamenu, text="Update or delete a Metadataset", font=("Helvetica", 16, "bold"), anchor="w")
    ...
    data_menu.add_separator()
    data_menu.add_command(label="Delete Dataset", command=delete_metadata)
    ...
    current_file_path = os.path.join(metadata_tables_path, selected_file)
   ...
   # Back button
    ...
    # Keep main menu visible during loading and hide it only once this window is ready.
    if root is not None and root.winfo_exists():
        root.withdraw()

    newdatamenu.protocol("WM_DELETE_WINDOW", go_back)
