...
#Parameters for test
test_id = "https://zenodo.org/records/12514950"
...
def get_was_issued(data):
    was_issued = data['metadata']["publication_date"]
    return was_issued
...
#set manually
def get_owner(data):
    has_owner = data['metadata']["creators"][0]["name"]
    has_owner = has_owner.split(", ")
    fist_name = has_owner[1]
    last_name = has_owner[0]
    has_owner = f"{fist_name} {last_name}"
    return has_owner
...
def get_native_subject(data):
    subjects = data['metadata']["keywords"]
    if len(subjects) > 0:
...
has_native_subject = get_native_subject(data)
...
def get_access_rights(data):
    rights = data['metadata']['license']['id']
    return rights
